...
export default class CompFactory {
    ...
    text(startTime = 0, duration = 10, content = "", colorHax = "#000000", x = 0, y = 0, dynWidth = 20, dynHeight = 20) {
    ...
    Image(startTime = 0, duration = 300, imgId, x = 0, y = 0) {
        let bs = new Image(startTime, duration, imgId, x, y);
        this.insert(bs);
        return bs;
    }
    line(startTime = 0, duration = 300, x1 = 0, y1 = 0, x2 = 100, y2 = 100, color = "black", lineWidth = 2) {
        let bs = new Line(startTime, duration, x1, y1, x2, y2, color, lineWidth);
        this.insert(bs);
        return bs;
    }
    circle(startTime = 0, duration = 300, x = 0, y = 0, radiusPercent = 10, color = "grey", fill = true, startAngle = 0, endAngle = 2 * Math.PI) {
        let item = new Circle(startTime, duration, x, y, radiusPercent, color, fill, startAngle, endAngle);
        this.insert(item);
        return item;
    }
}
