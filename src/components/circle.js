...
export default class Circle extends BaseComponent {
    ...
    draw(p) {
        ...
        p.beginPath();
        p.drawCircle(this.contentX() + (this.width.value() / 2), this.contentY() + (this.width.value() / 2), (this.width.value() / 2), this.filled.value(), this.startAngle.value() * (Math.PI / 180), this.endAngle.value() * (Math.PI / 180), this.style);
        p.stroke();
        this.postDraw(p);
        return true;
    }
